...
class GlocalRuntime {
  ...
  static Future<GlocalRuntime> bootstrap() async {
    ...
    return GlocalRuntime._(
      ...
      settingsService: SettingsService(HiveSettingsRepository.fromHive()),
      locationProfileService: LocationProfileService(),
      runtimeBox: Hive.box<dynamic>(HiveBootstrap.runtimeBox),
    );
  }

  Future<void> recoverMissedReminders() async {
    final now = DateTime.now();
    final lastRecoveryAt = (_runtimeBox.get(_lastRecoveryAtKey) as num?)?.toInt();
  ...
  Future<void> _runReminderCycle(DateTime now, UserSettings settings) async {
    ...
    final slot = now.millisecondsSinceEpoch ~/ reminderCheckEvery.inMilliseconds;
  ...
  Future<void> _runHourlyCycle(DateTime now, UserSettings settings) async {
    final hourKey = (now.year * 1000000) +
        (now.month * 10000) +
        (now.day * 100) +
        now.hour;
    final lastHourKey =
    ...
    await _announcementService.speakHourlyTime(now, settings);
    await _announcementService.speakWeather(weather, settings, now);
  }
}
